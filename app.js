  ...
  $("#app").innerHTML = `
    ${card(`
      ...
      <div class="toolbar">
        ...
        <div class="btn" style="flex:1">Checked Total: $<span id="gCheckedTotal">${money(checked.total)}</span> (<span id="gCheckedCount">${checked.count}</span>)</div>
    ...
    <div class="stickyTotals" id="sStickyTotals">
      <div class="stickyTotalsInner">
        ...
        <div class="stItem">Running: $<span id="gTotalSticky">${money(total)}</span></div>
      </div>
    </div>
    <div class="footerSpace"></div>
  `;
  wireChecks();
  wirePrices("groceryPrices", "#gTotal, #gTotalSticky");
  ...
  $("#app").innerHTML = `
    ${card(`
      ...
      <div class="toolbar">
        ...
        <div class="btn" style="flex:1">Checked Total: $<span id="sCheckedTotal">${money(checkedTotal)}</span> (<span id="sCheckedCount">${checkedCount}</span>)</div>
    ...
    <div class="stickyTotals" id="sStickyTotals">
      <div class="stickyTotalsInner">
        ...
        <div class="stItem">Running: $<span id="sTotalSticky">${money(total)}</span></div>
      </div>
    </div>
    <div class="footerSpace"></div>
  `;
...
function wireQty(bucket="qtyTotals"){
...
async function renderGroceryStorage(){
  const data = await getData();
  setHeader("Grocery Storage", "Check off what you’ve stored and track quantities.");

  const rows = data.grocery.map((name, idx) => {
    const id = `gstore:${idx}`;
    const qtyId = `gstoreQty:${idx}`;
    const qtyVal = state.qtyTotals[qtyId] ?? "";
    const extra = `
      <div class="priceRow">
        <label class="itemMeta" style="min-width:90px">Qty</label>
        <input class="input" inputmode="numeric" placeholder="0" value="${escapeHtml(qtyVal)}" data-qty="${qtyId}"/>
      </div>`;
    return checkRow(id, name, "", extra);
  }).join("");
  ...
  $("#app").innerHTML = `
    ${card(`
      <div class="h1">Grocery Storage Checklist</div>
      <p class="sub">Use this after shopping or meal prep to track what’s in your pantry/fridge/freezer.</p>
      <div class="pillRow" style="margin-top:10px">
        <a class="pill" href="#grocery">Back to Grocery</a>
      </div>
      ${rows}
    `)}
    <div class="footerSpace"></div>
...
async function renderSuppStorage(){
  const data = await getData();
  setHeader("Supplement Storage", "Check off what you have and track quantities.");

  const sections = [];
  const cats = data.supplements || {};
  Object.keys(cats).forEach(catName => {
    const arr = cats[catName] || [];
    const rows = arr.map((name, idx) => {
      const id = `sstore:${catName}:${idx}`;
      const qtyId = `sstoreQty:${catName}:${idx}`;
      const qtyVal = state.qtyTotals[qtyId] ?? "";
      const extra = `
        <div class="priceRow">
          <label class="itemMeta" style="min-width:90px">Qty</label>
      ...
      return checkRow(id, name, catName, extra);
    }).join("");
    sections.push(card(`
      <div class="h1" style="font-size:16px">${escapeHtml(catName)}</div>
      ${rows}
    `));
  });
  ...
  $("#app").innerHTML = `
    ${card(`
      ...
      <p class="sub">Track what you have stocked, plus quantities on hand.</p>
      <div class="pillRow" style="margin-top:10px">
        <a class="pill" href="#supplements">Back to Supplements</a>
      </div>
    `)}
    ${sections.join("")}
    <div class="footerSpace"></div>
  ...
  $("#app").innerHTML = `
    ${card(`
      ...
      <div class="pillRow">
        ...
        <a class="pill" href="#groceryStorage">Grocery Storage</a>
